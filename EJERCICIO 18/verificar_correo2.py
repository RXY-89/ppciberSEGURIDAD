...
if response.status_code == 200:
    ...
    try:
            ...
            for i, brecha in enumerate(brechas[:3]):
                ...
                try:
                    ...
                    if detalle_resp.status_code == 200:
                        ...
                        msj = (
                            f"No se pudo obtener detalles de {nombre}."
                            f"Código: {detalle_resp.status_code}"
                        )
                        logging.error(msj)
                except Exception as e:
                    error_msg = (
                        f"Error al consultar detalles de {nombre}: {e}"
                    )
        ...
        sys.exit(1)
    print("Consulta completada. Revisa reporte.csv para los resultados.")
    ...
    error_msg = f"Error inesperado. Código: {response.status_code}"
    print(error_msg)
    logging.error(error_msg)
